...
The card carries one of the two surfaces in `components/dashboard/cardSurface.ts`, chosen by whether the viewer can actually open the workspace — `ACTIVE_SURFACE` when `joinDenialLabel` returns `null`, `PANEL_SURFACE` (identical to the `Usage` and `GettingStarted` panels) otherwise.

- That single condition already encodes the role split, so nothing here tests for role. `joinDenialLabel` returns `null` for a host at phase `ready` but holds a student at `"Waiting for your tutor"` until phase `open` — so a **tutor** gets the raised card as soon as the room is ready, a **student** only once the tutor has opened it.
- The raised state is `--card-background-hover` behind a `--foreground/50` border, brightening to full `--foreground` on hover. Those two border values are the resting and hover states of the external-link icon in the card's top right, which is `--foreground` at `opacity-50`/`opacity-100` — the border and the icon are deliberately one signal, so both are suppressed together when the card is not openable.
- The skeleton uses `PANEL_SURFACE`: a lesson is usually not joinable, so settling into the calm state flashes less often than settling out of the raised one.
...
`components/dashboard/Usage.tsx` is the second card in `DashboardCardRow` **wherever the checklist is not**, so the top of the dashboard is always either `Next` + checklist, `Next` + usage, or `Next` alone.

- **One `companion` node decides it.** `DashboardClient` resolves the checklist, the usage card, or `null` in a single expression, and the row renders `<Next paired />` beside whatever came back. The two cards therefore cannot both appear by construction rather than by a pair of conditions that have to be kept opposite. The rule it encodes: usage is meaningless to someone who has not run a lesson yet, and the checklist is the only thing worth the space until they have.
- **The gate is `presentation === "hidden"` and `entitlements !== null`.** The first half is the checklist's own retirement signal (`startedCount > 0`, or admin); the second is what makes this tutor-only without a role check — students have no plan row, and `unpaid` / `cancelled` resolve to no entitlements. All of them fall through to the lone `Next`, which is what the page already did.
- **It borrows `PANEL_CLASS` from `GettingStarted`**, the same way `GettingStarted` borrows `nextCardWidth` from `Next`. The card that established a look exports it; the usage card is deliberately the plain panel and not the `gradient-border` treatment, because `Next` is the hero of that row and two gradient cards would compete.
- **The tier name arrives as a `planId` prop, not on `EntitlementsState`**, which carries numbers only. `app/dashboard/page.tsx` resolves it with `grantedPlanForUser` and `DashboardClient` hands it to the card alongside `plan` — see [docs/plans.md](plans.md) for the badge itself and what a missing tier means.
- **The meters are data**, derived in `lib/planUsage.ts` (`resolveUsageMeters`, `planFactsLine`) so the component renders a `UsageMeter[]` and nothing else. A `null` limit — Professional's workspaces and linked students — renders a bare tally and **no bar**: a full track would read as "at the limit", and an empty one as "none used".
- **The reset date is the billing period's end, not the month's.** It comes from `usage.periodEnd` through `resetLabel`, now exported from `lib/planDenialCopy.ts`, so the card and the quota denial toast name the same date in the same words. Keep them on that one helper.
- **The linked-students count is not `friends.length`.** `listLinksFor` returns deactivated links as well, so the dashboard's own list would tally `4 / 3` after a downgrade. `countActiveStudentLinks` counts the active ones server-side in `app/dashboard/page.tsx` and travels on `EntitlementsState` alongside `usage`. It is resolved **only when entitlements exist**, so a student's dashboard does not pay for a query counting links they cannot have, and a failed count yields `null`, which drops the row rather than showing a wrong number.
- **At the limit the tally and the fill go `destructive`**, and the caption names the way out — the reset date for workspaces, "remove one to link another" for students.
- **"Change plan" is disabled behind an `Upcoming` badge**, the same treatment the sidebar's Messages row carries. There is no billing flow yet; plan changes are a `user_plans` write plus a reconcile call.
- **The button is absent on Professional**, not disabled twice over. There is nothing above the top tier, so a full-width control promising one is a pitch aimed at someone who has already bought everything — and the card is the wrong surface for "move down a tier" at that weight. Changing plan in either direction belongs in Settings when it exists; this button is a placeholder for the upgrade path only, and should be deleted rather than rehomed once Settings can carry both.
- `DashboardSkeleton` still renders a lone `NextSkeleton`. It cannot predict the pairing for the usage card any more than it can for the checklist, because both depend on counts that are still in flight — see the loading note in the checklist section above.
