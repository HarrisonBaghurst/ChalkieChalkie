...
- `app/api/` — Backend routes:
    ...
    - `admin/reconcile-plan` — applies a plan change to everything the user already owns. POST `{ userId }`, guarded by `requireAdmin` and deliberately unrated; the Stripe webhook will call the same `reconcilePlanChange` once it exists
    ...
    - `tutor-access` — the tutor-access request from the getting-started checklist. Unlike `contact` it **requires auth**, because the point of the request is to identify the account to promote: it reads `auth()`, ignores the client's `title`, and stamps the Clerk ID, account email, name and current role into the email itself rather than trusting the form. Its own `tutor-access` rate-limit key (2/day/user) so a request cannot be spent on `contact`'s 3/hour/IP budget
