...
- **The room's member cap is enforced on connect, off the signed ticket.** `BoardRoom.fetch` counts *distinct users* rather than sockets — `MAX_CONNECTIONS_PER_USER` already allows one person three tabs — and the host bypasses the check so a dropped tutor can always re-enter their own full room. A refused joiner is accepted and immediately closed with `ROOM_FULL_CLOSE_CODE` (4004), because a browser cannot read an HTTP status off a failed handshake; `client.ts` stops reconnecting on that code, which is what stops a full room becoming a reconnect loop. Full reasoning in [docs/plans.md](plans.md).
...
- A selecting user's **cursor is replaced by their name pill**, pinned to the box in `CursorLayer.tsx`. The pill deliberately has no CSS transition, unlike the cursor: the box it labels is canvas-drawn without one. It carries no avatar: the pill is already filled with the owner's `getUserColour`, so one inside it would be invisible.
- **Colour is never on the wire.** `UserInfo` carries `firstName` / `lastName` / `email` and nothing else; every consumer derives the colour client-side from `getUserColour(userId)` (`lib/userColour.ts`), so cursor, name pill, selection outline, roster dot and dashboard avatar all agree without the Worker knowing colour exists.
